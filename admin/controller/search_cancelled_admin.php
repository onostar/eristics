<?php
    ...
    $select_date = $connectdb->prepare("SELECT users.first_name, users.last_name, users.email, users.address, users.city, orders.order_id, orders.customer_email, orders.item_name, orders.quantity, orders.item_price, orders.restaurant, orders.order_date, orders.delivery_date FROM users, orders WHERE users.email = orders.customer_email AND orders.order_status = -1 AND orders.delivery_date BETWEEN '$from_date' AND '$to_date' ORDER BY orders.order_date");
