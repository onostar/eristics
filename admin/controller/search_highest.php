<?php
    ...
    $select_items = $connectdb->prepare("SELECT item_name, item_price, restaurant, delivery_date, SUM(quantity) AS total_quantity, SUM(item_price) AS total_amount FROM orders WHERE order_status = 1 AND delivery_date BETWEEN '$from_date' AND '$to_date' GROUP BY item_name ORDER BY total_quantity DESC");
